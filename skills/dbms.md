...
* **Homogeneous distributed database**: These systems use the same DBMS software from multiple sites. Data exchange between these various sites can be handled easily. For example, library information systems by the same vendor, such as Geac Computer Corporation, use the same DBMS software which allows easy data exchange between the various Geac library sites.
* **Heterogeneous distributed database systems**: In this system, different sites might use different DBMS software, but there is additional common software to support data exchange between these sites. For example, the various library database systems use the same machine-readable cataloguing (MARC) format to support library record data exchange.
...
![Multi-access-image](https://cdn.goskills.com/blobs/blogs/262/8.jpg) - A database management system makes it possible for multiple authorized users to access the same database from different places, in different ways, to fulfill different intents.
